 % Parameters
c = 1.0;                   % Chord length
epsilon_over_c = 0.02;     % Maximum camber to chord ratio
angle_of_attack = 7:-1:-7;  % Angle of attack in degrees
m_values = [4 10 30 100];  % Number of panels
Vinf = 1;

% Generate airfoil data
for idx = 1:length(m_values)
    m = m_values(idx);
    filename = sprintf('airfoil_data_m%d.txt', m);
...
for idx = 1:length(m_values)
    ...
    CL = flip(CL);
    % Plot the results
    plot(angle_of_attack, CL, 'Color', colors{idx}, 'Marker', markers{idx}, 'DisplayName', sprintf('m = %d', m));
end

%Plotting thin air_foil Theory curve
aoa_range_rad = deg2rad(angle_of_attack);
CL_thin_airfoil = 2 * pi * (aoa_range_rad + epsilon_over_c);
plot(angle_of_attack, CL_thin_airfoil, '-.k', 'DisplayName', 'Thin Airfoil Theory', 'LineWidth', 1.5);
